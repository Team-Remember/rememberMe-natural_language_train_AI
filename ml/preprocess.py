...
# 카카오톡 파일 전처리
def open_and_preprocess_kakao_file(files):
    ...
    # 전처리
    chat_room = katalk_parse(kakao_before_contents)
    # 전처리 결과 append
...
# 카카오톡 데이터 전처리
def katalk_parse(kakao_before_contents):
    my_katalk_data = list()
...
# input 데이터프레임으로 변형
def preprocess_db_data(db_data):
    db_list = []
